...
namespace MySecondAngular.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class TopAnswersController : ControllerBase
    {
        private readonly ILogger<TopAnswersController> _logger;

        public TopAnswersController(ILogger<TopAnswersController> logger)
        {
            _logger = logger;
        }
        ...
        public List<Comment> Get()
        {
            ...
            SqlDataAdapter Adapter = new SqlDataAdapter($"Select Top 5 * From Comments Order By ScoreUP DESC", SQL.con);
